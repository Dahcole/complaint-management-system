...
<div class="col-lg-6">
    <div class="card">
        <div class="card-body">
            <h4 class="card-title">Latest Complaints </h4>
        </div>
        <div class="comment-widgets scrollable">
            <?php   $redirectAll = $conn->base_url()."complaints"; ?>
            ...
            <?php
            $sql = "SELECT studentID FROM user WHERE username =:user";
            $conn->query($sql);
            $conn->bind(":user", $_SESSION['username']);
            $rowCount = $conn->rowCount();

            if($rowCount > 0){
                $student_id = $conn->fetchColumn();
                $sql = "SELECT * FROM complaints JOIN user u on complaints.studentID = u.studentID
                                 WHERE complaints.studentID = :id ORDER BY id DESC LIMIT 4";
                $conn->query($sql);
                $conn->bind(":id", $student_id);
                ...
                foreach ($result_set as $complaint) {
                    ...
                    $redirect = $conn->base_url()."view-complaint/$complaint_no";
                ...
                echo "<p>You have not added or received replies for any complaints yet. </p>";
            }
...
<div class="col-lg-6">
    ...
    <div class="card">
        <div class="card-body">
            <h4 class="card-title">Latest Complaint History</h4>
                ...
                <ul class="chat-list">
                    <?php
                     ...
                     if($rowCount > 0){
                         ...
                         $sql = "SELECT * FROM complaints JOIN user u on complaints.studentID = u.studentID
                                 WHERE complaints.studentID = :id ORDER BY id DESC LIMIT 1";
                         $conn->query($sql);
                         $conn->bind(":id", $student_id);
                    ...
                    $sql = "SELECT * FROM complaint_replies JOIN complaints on complaint_replies.student_id = complaints.studentID 
                                                ...
                                                WHERE studentID =:student_id AND complaint_no =:complaint_no ORDER BY complaint_replies.id DESC";
                    $conn->query($sql);
                    ...
                    if($row_count > 0){
                        foreach ($result_details as $replies){
                            ...
                            if($replied_by === $name){
                                ?>
                                <li class="chat-item">
                                    <div class="chat-img"><i class="fa fa-user" style="font-size: 2.2em; padding: 2px 15px;"></i></div>
                                    <div class="chat-content">
