...
<head>
     <title> Edit Profile | Complaints Management System - Yaba College of Technology </title>
...
<?php include 'inc/preloader.php'; ?>
                        ...
                        <div class="card">
                          <form class="form-horizontal" method="post">
                                <div class="card-body">
                                    ...
                                    <div class="form-group row">
                                        <label for="matricno" class="col-sm-3 control-label col-form-label">Matric No</label>
                                        <div class="col-sm-9">
                                            <input type="text" class="form-control" id="matricno" value="<?php echo $matric_no; ?>" name="matric_no" required>
                                        </div>
                                    </div>
                                    <div class="form-group row">
                                       ...
                                       <div class="form-group row">
                                        ...
                                        <div class="col-sm-9">
                                            <input type="text" class="form-control" name="username" id="username" value="<?php echo $_SESSION['username']; ?>" readonly style="cursor: not-allowed;">
                                        </div>
                                    </div>

                                    <div class="form-group row">
                                        <label for="cono1" class="col-sm-3 control-label col-form-label">Account Status</label>
                                        <div class="col-sm-9">
                                            <?php
                                             if($status == 'active'){
                                                 ?> <span style="color: mediumseagreen; position: relative; top: 6px;"> <i class="fa fa-check"></i>Active</span><?php
                                             }
            ...
            <?php
              if(isset($_POST['update_profile'])){
                   ...
                   $email =$_POST['email'];
                    ...
                    $redirect = $conn->base_url().'profile';
    ...
    if($conn->rowCount() > 0){
        echo "<script>
              toastr['error']('Email already exists for another user.');
           </script>";
        return false;
    }
    ...
    $sql = "SELECT * FROM user WHERE matric_no =:no  AND username !=:username";
    $conn->query($sql);
    $conn->bind(":no", $matric_no);
    $conn->bind(":username", $_SESSION['username']);
    if($conn->rowCount() > 0){
        echo "<script>
              toastr['error']('Matric No already exists for another user.');
           </script>";
        return false;
    }else {
        ...
        if($conn-> validate_phone($phone)){
              $sql = "UPDATE user SET matric_no =:matric_no, name =:name, phone_no=:phone,email=:email WHERE username =:username ";
        $conn->query($sql);
        $conn->bind(":name", $full_name);
        $conn->bind(":email", $email);
